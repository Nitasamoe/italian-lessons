# Italian Grammar — A1/B1 Level

<!-- INDEX (line ranges for selective reading)
§1   Presente Indicativo .............. lines 22–70
§1b  Irregular Verbs (presente) ...... lines 72–182
§2   Articolo Determinativo .......... lines 184–208
§3   Articolo Indeterminativo (sg) ... lines 210–218
§4   Articolo Indeterminativo (pl) ... lines 220–226
§5   Passato Prossimo ................ lines 228–235
§6   Futuro Semplice ................. lines 237–268
§7   Futuro Anteriore ................ lines 270–280
§8   Pronomi Soggetto ................ lines 282–292
§9   Pronomi Riflessivi .............. lines 294–304
§10  Pronomi Diretti ................. lines 306–330
§11  Pronomi Indiretti ............... lines 332–354
§12  Reflexive Verbs ................. lines 356–362
§13  Volerci / Metterci .............. lines 364–381
§14  CI ............................... lines 383–390
§15  Ne ............................... lines 392–396
§16  Comparatives & Superlatives ..... lines 398–412
§17  Conjunctions .................... lines 414–418
§18  Subordinate Clause Construction . lines 420–451
§19  Relative Pronouns ............... lines 453–470
§20  Sentence Connectors & Discourse . lines 472–490
Basic Sentence Structure ............ lines 492–497
Negation ............................ lines 499–500
Common Prepositions ................. lines 502–513
-->

## 1. Presente Indicativo

### essere (to be)

| Person     | Form    |
|------------|---------|
| io         | sono    |
| tu         | sei     |
| lui/lei    | è       |
| noi        | siamo   |
| voi        | siete   |
| loro       | sono    |

### avere (to have)

| Person     | Form    |
|------------|---------|
| io         | ho      |
| tu         | hai     |
| lui/lei    | ha      |
| noi        | abbiamo |
| voi        | avete   |
| loro       | hanno   |

### Regular -are verbs (e.g. parlare)

| Person     | Ending | Example   |
|------------|--------|-----------|
| io         | -o     | parlo     |
| tu         | -i     | parli     |
| lui/lei    | -a     | parla     |
| noi        | -iamo  | parliamo  |
| voi        | -ate   | parlate   |
| loro       | -ano   | parlano   |

### Regular -ere verbs (e.g. credere)

| Person     | Ending | Example   |
|------------|--------|-----------|
| io         | -o     | credo     |
| tu         | -i     | credi     |
| lui/lei    | -e     | crede     |
| noi        | -iamo  | crediamo  |
| voi        | -ete   | credete   |
| loro       | -ono   | credono   |

### Regular -ire verbs (e.g. dormire)

| Person     | Ending | Example   |
|------------|--------|-----------|
| io         | -o     | dormo     |
| tu         | -i     | dormi     |
| lui/lei    | -e     | dorme     |
| noi        | -iamo  | dormiamo  |
| voi        | -ite   | dormite   |
| loro       | -ono   | dormono   |

### -ire verbs with -isc- (e.g. capire)

| Person     | Form      |
|------------|-----------|
| io         | capisco   |
| tu         | capisci   |
| lui/lei    | capisce   |
| noi        | capiamo   |
| voi        | capite    |
| loro       | capiscono |

## 1b. Irregular Verb Conjugations — Presente Indicativo

### andare (to go)

| Person  | Form    |
|---------|---------|
| io      | vado    |
| tu      | vai     |
| lui/lei | va      |
| noi     | andiamo |
| voi     | andate  |
| loro    | vanno   |

### fare (to do/make)

| Person  | Form     |
|---------|----------|
| io      | faccio   |
| tu      | fai      |
| lui/lei | fa       |
| noi     | facciamo |
| voi     | fate     |
| loro    | fanno    |

### bere (to drink)

| Person  | Form    |
|---------|---------|
| io      | bevo    |
| tu      | bevi    |
| lui/lei | beve    |
| noi     | beviamo |
| voi     | bevete  |
| loro    | bevono  |

### stare (to stay/be)

| Person  | Form   |
|---------|--------|
| io      | sto    |
| tu      | stai   |
| lui/lei | sta    |
| noi     | stiamo |
| voi     | state  |
| loro    | stanno |

### venire (to come)

| Person  | Form    |
|---------|---------|
| io      | vengo   |
| tu      | vieni   |
| lui/lei | viene   |
| noi     | veniamo |
| voi     | venite  |
| loro    | vengono |

### dire (to say/tell)

| Person  | Form    |
|---------|---------|
| io      | dico    |
| tu      | dici    |
| lui/lei | dice    |
| noi     | diciamo |
| voi     | dite    |
| loro    | dicono  |

### potere (can/to be able to)

| Person  | Form     |
|---------|----------|
| io      | posso    |
| tu      | puoi     |
| lui/lei | può      |
| noi     | possiamo |
| voi     | potete   |
| loro    | possono  |

### volere (to want)

| Person  | Form     |
|---------|----------|
| io      | voglio   |
| tu      | vuoi     |
| lui/lei | vuole    |
| noi     | vogliamo |
| voi     | volete   |
| loro    | vogliono |

### dovere (must/to have to)

| Person  | Form     |
|---------|----------|
| io      | devo     |
| tu      | devi     |
| lui/lei | deve     |
| noi     | dobbiamo |
| voi     | dovete   |
| loro    | devono   |

### vedere (to see)

| Person  | Form    |
|---------|---------|
| io      | vedo    |
| tu      | vedi    |
| lui/lei | vede    |
| noi     | vediamo |
| voi     | vedete  |
| loro    | vedono  |

## 2. Articolo Determinativo (Definite Articles)

### Masculine Singular
| Article | Usage                                    | Example        |
|---------|------------------------------------------|----------------|
| il      | before most consonants                   | il libro       |
| lo      | before s+consonant, z, gn, ps, x, y     | lo studente, lo zaino |
| l'      | before a vowel                           | l'amico        |

### Masculine Plural
| Article | Usage                                    | Example        |
|---------|------------------------------------------|----------------|
| i       | plural of il                             | i libri        |
| gli     | plural of lo and l'                      | gli studenti, gli amici |

### Feminine Singular
| Article | Usage                                    | Example        |
|---------|------------------------------------------|----------------|
| la      | before consonants                        | la casa        |
| l'      | before a vowel                           | l'amica        |

### Feminine Plural
| Article | Usage                                    | Example        |
|---------|------------------------------------------|----------------|
| le      | for all feminine plurals                 | le case, le amiche |

## 3. Articolo Indeterminativo — Singular (Indefinite Articles)

| Gender    | Form  | Usage                              | Example          |
|-----------|-------|------------------------------------|------------------|
| Masculine | un    | before most words                  | un libro, un amico |
| Masculine | uno   | before s+consonant, z, gn, ps, x, y | uno studente, uno zaino |
| Feminine  | una   | before consonants                  | una casa         |
| Feminine  | un'   | before vowels                      | un'amica         |

## 4. Articolo Indeterminativo — Plural (Partitive Articles)

| Gender    | Form   | Usage                                              | Example                    |
|-----------|--------|----------------------------------------------------|----------------------------|
| Masculine | dei    | before most words                                  | dei libri, dei ragazzi     |
| Masculine | degli  | before s+consonant, z, gn, ps, x, y, or a vowel   | degli studenti, degli amici |
| Feminine  | delle  | before all feminine plural nouns                   | delle case, delle amiche   |

## 5. Passato Prossimo

Auxiliary verb (avere/essere) in present tense + participio passato.

**Auxiliary choice:**
- **avere** = most verbs: *Ho parlato, abbiamo dormito.*
- **essere** = verbs of motion, change of state, reflexives: *Sono arrivato, è diventata, mi sono svegliato.*

With **essere**, the past participle agrees in gender/number with the subject.

## 6. Futuro Semplice

### -are verbs (parler-)
| Person     | Form       |
|------------|------------|
| io         | parlerò    |
| tu         | parlerai   |
| lui/lei    | parlerà    |
| noi        | parleremo  |
| voi        | parlerete  |
| loro       | parleranno |

### -ere verbs (creder-)
| Person     | Form        |
|------------|-------------|
| io         | crederò     |
| tu         | crederai    |
| lui/lei    | crederà     |
| noi        | crederemo   |
| voi        | crederete   |
| loro       | crederanno  |

### -ire verbs (dormir-)
| Person     | Form        |
|------------|-------------|
| io         | dormirò     |
| tu         | dormirai    |
| lui/lei    | dormirà     |
| noi        | dormiremo   |
| voi        | dormirete   |
| loro       | dormiranno  |

## 7. Futuro Anteriore

Auxiliary verb in futuro semplice + participio passato.

| Auxiliary | Forms                                          |
|-----------|-------------------------------------------------|
| avere     | avrò, avrai, avrà, avremo, avrete, avranno     |
| essere    | sarò, sarai, sarà, saremo, sarete, saranno      |

Often used after **appena** and **quando**:
*Quando avrò finito, uscirò.*

## 8. Pronomi Soggetto

| Italian | German    |
|---------|-----------|
| io      | ich       |
| tu      | du        |
| lui     | er        |
| lei     | sie       |
| noi     | wir       |
| voi     | ihr       |
| loro    | sie (Pl.) |

## 9. Pronomi Riflessivi

| Person  | Pronoun |
|---------|---------|
| io      | mi      |
| tu      | ti      |
| lui/lei | si      |
| noi     | ci      |
| voi     | vi      |
| loro    | si      |

Example: *mi alzo, ti alzi, si alza, ci alziamo, vi alzate, si alzano*

## 10. Pronomi Diretti (Direct Object Pronouns)

### Atonic (before verb)
| Person     | Pronoun |
|------------|---------|
| io         | mi      |
| tu         | ti      |
| lui        | lo      |
| lei        | la      |
| noi        | ci      |
| voi        | vi      |
| loro (m.)  | li      |
| loro (f.)  | le      |

### Tonic / stressed (after prepositions or for emphasis)
| Person     | Pronoun  |
|------------|----------|
| io         | me       |
| tu         | te       |
| lui/lei    | lui/lei  |
| noi        | noi      |
| voi        | voi      |
| loro       | loro     |

Used after prepositions (per, con, senza, tra, su) or to emphasize the object.

## 11. Pronomi Indiretti (Indirect Object Pronouns)

### Atonic (before verb)
| Person     | Pronoun |
|------------|---------|
| io         | mi      |
| tu         | ti      |
| lui        | gli     |
| lei        | le      |
| noi        | ci      |
| voi        | vi      |
| loro       | gli     |

### Tonic / stressed
| Person     | Pronoun   |
|------------|-----------|
| io         | a me      |
| tu         | a te      |
| lui/lei    | a lui/lei |
| noi        | a noi     |
| voi        | a voi     |
| loro       | a loro    |

## 12. Reflexive Verbs (Riflessivi)

Conjugation pattern (e.g. alzarsi):
*mi alzo – ti alzi – si alza – ci alziamo – vi alzate – si alzano*

In passato prossimo, reflexives always use **essere**:
*Mi sono alzato/a.*

## 13. Volerci / Metterci

### volerci (impersonal — "it takes")
No subject, always introduced by **ci**:
- *Ci vuole un'ora per andare a Roma.* (singular)
- *Ci vogliono due ore.* (plural)

Used for: time, ingredients, needs.
*Per andare in Cina ci vuole il passaporto.*

### metterci (personal — "to take [time]")
Has a subject:
- *Io ci metto 2 ore per andare a Roma.*
- *Il treno ci mette 20 minuti.*
- *Il treno ci ha messo 30 minuti.* (passato prossimo)

Personal need uses **bisogna** or **avere bisogno di**:
*Ho bisogno del passaporto per andare in Cina.*

## 14. CI

Multiple uses:
1. **Location** — "here/there": *Ci vado domani.* (I'm going there tomorrow.)
2. **volerci / metterci** — see above
3. **Reflexive pronoun** — noi: *Ci alziamo.*
4. **Direct object pronoun** — noi: *Ci vedono.*
5. **Indirect object pronoun** — a noi: *Ci parlano.*

## 15. Ne

Used for quantities or replacing **di + something**:
- *Ne voglio due.* (I want two of them.)
- *Parli spesso di politica? — No, non ne parlo mai.*

## 16. Comparatives and Superlatives

**Comparative:** più / meno + adjective + di
- *Maria è più alta di Luca.*
- *Questo libro è meno interessante di quello.*

**Superlative:** il/la più / il/la meno + adjective + di/del
- *È il più bello del gruppo.*

### Irregular forms
| Adjective | Comparative | Superlative   |
|-----------|-------------|---------------|
| buono     | migliore    | il migliore   |
| cattivo   | peggiore    | il peggiore   |
| grande    | maggiore    | il maggiore   |
| piccolo   | minore      | il minore     |

## 17. Conjunctions

See vocabulary.md for the full list. Key grammar notes:
- Coordinating conjunctions connect independent clauses: *Parlo italiano, ma non lo scrivo bene.*
- Subordinating conjunctions introduce dependent clauses: *Studio perché voglio imparare.*

## 18. Subordinate Clause Construction

### Word order
Italian subordinate clauses keep normal SVO order (unlike German):
- *Studio perché voglio imparare.* (I study because I want to learn.)
- *Quando piove, resto a casa.* (When it rains, I stay home.)

### Comma rule
When the subordinate clause comes **first**, use a comma before the main clause:
- *Quando arrivo, ti chiamo.* ← subordinate first → comma
- *Ti chiamo quando arrivo.* ← main clause first → no comma needed

### Tense pairing

| Conjunction | Subordinate tense | Main clause tense | Example |
|-------------|-------------------|-------------------|---------|
| quando      | presente          | presente          | *Quando piove, resto a casa.* |
| quando      | futuro            | futuro            | *Quando arriverò, ti chiamerò.* |
| se          | presente          | presente/futuro   | *Se piove, restiamo a casa.* / *Se piove, resteremo a casa.* |
| mentre      | presente          | presente          | *Mentre studio, ascolto la musica.* |
| perché      | presente          | presente          | *Mangio perché ho fame.* |

### Infinitive constructions (preposition + infinitive)

| Construction     | Meaning         | Example |
|------------------|-----------------|---------|
| prima di + inf.  | before doing    | *Prima di uscire, chiudo la porta.* |
| dopo + inf.      | after doing     | *Dopo aver mangiato, leggo un libro.* |
| senza + inf.     | without doing   | *Esce senza parlare.* |
| per + inf.       | in order to     | *Studio per imparare l'italiano.* |

Note: *dopo* takes a past infinitive (*dopo aver mangiato*), while the others take a simple infinitive.

## 19. Relative Pronouns (Pronomi Relativi)

### che (who / that / which)
Used as **subject** or **direct object** of the relative clause. No preposition before it.
- Subject: *La ragazza che abita qui è mia amica.* (The girl who lives here is my friend.)
- Object: *Il libro che leggo è interessante.* (The book that I'm reading is interesting.)

### cui (whom / which — after prepositions)
Used after prepositions: **in cui, di cui, con cui, a cui, per cui**
- *La città in cui abito è bella.* (The city in which I live is beautiful.)
- *Il libro di cui parlo è nuovo.* (The book I'm talking about is new.)
- *L'amica con cui studio è brava.* (The friend with whom I study is good.)
- *La ragione per cui studio è il lavoro.* (The reason why I study is work.)

### Key difference: che vs cui
- **che** = no preposition needed → *La persona che vedo...*
- **cui** = always after a preposition → *La persona con cui parlo...*

## 20. Sentence Connectors & Discourse

### Adding nuance
| Italian    | German                     | Usage |
|------------|----------------------------|-------|
| infatti    | tatsächlich / in der Tat   | confirms what was just said: *È stanco, infatti va a dormire.* |
| comunque   | jedenfalls / trotzdem      | anyway/regardless: *Comunque, andiamo al cinema.* |
| inoltre    | außerdem / darüber hinaus  | adds information: *Inoltre, parlo anche tedesco.* |
| invece     | stattdessen / hingegen     | contrast: *Lui studia, io invece lavoro.* |
| cioè       | das heißt / also           | clarifies: *Studio lingue, cioè italiano e tedesco.* |
| per esempio | zum Beispiel              | gives example: *Mi piace la frutta, per esempio le fragole.* |

### Sequencing
| Italian    | German                     | Usage |
|------------|----------------------------|-------|
| prima      | zuerst                     | *Prima mangio, poi esco.* |
| poi        | dann / danach              | *Poi andiamo al parco.* |
| dopo       | danach / nachher           | *Dopo studiamo insieme.* |
| infine     | schließlich / zum Schluss  | *Infine, andiamo a dormire.* |
| alla fine  | am Ende                    | *Alla fine abbiamo capito.* |

## Basic Sentence Structure

- Subject + Verb + Object: *Io mangio la pizza.*
- Subject pronouns are often omitted: *Mangio la pizza.*
- Questions can be formed by intonation alone: *Mangi la pizza?*

## Negation

- Place **non** before the verb: *Non parlo italiano.*

## Common Prepositions

| Italian | German  | Example                    |
|---------|---------|----------------------------|
| a       | zu/nach | Vado a scuola.             |
| di      | von     | Il libro di Marco.         |
| in      | in      | Sono in casa.              |
| con     | mit     | Parlo con Maria.           |
| per     | für     | Questo è per te.           |
| da      | von/bei | Vengo da Roma.             |
| su      | auf     | Il libro è sul tavolo.     |
